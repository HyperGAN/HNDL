...
def _relation(s):
    ...
    if rank is None:
        return
    if dim >= rank:
        s.error("E_ARGUMENT", f"sum dim {dim} is outside rank {rank}")
    out_rank = rank if keepdim else rank - 1
    if out_rank < min(SUPPORTED_RANKS):
        s.error("E_CONSTRAINT", f"sum over dim {dim} of a rank-{rank} tensor would produce rank {out_rank}; "
    ...
    if rank not in SUPPORTED_RANKS:
        s.error("E_CONSTRAINT", f"sum input rank {rank} is unsupported; HNDL tensors have rank "
...
def _reference(module):
    def total(x):
        return x.sum(dim=module.dim, keepdim=module.keepdim)
    return total


@operator(
    "sum",
    summary="Add up one non-batch axis.",
    shape="x -> out",
    relation=_relation,
    ...
    args={
        "dim": Arg(int, min=1, max=max(SUPPORTED_RANKS) - 1,
                   help="Axis to sum over. The batch axis 0 cannot be reduced, so dim starts at 1."),
    ...
    examples=[
        ...
        Example("sum(1, keepdim=True)", ("B", 4, 6), ("B", 1, 6),

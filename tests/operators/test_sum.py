"""Numerics, bidirectional inference, and error codes for the ``sum`` reduction."""
...
def source(dim, keepdim):
    return f"sum({dim}, keepdim=True)" if keepdim else f"sum({dim})"
...
@pytest.mark.parametrize("device", DEVICES)
def test_repeated_reductions_total_an_image(device):
    plan = resolve("sum(2)\nsum(2)", input_shape=("B", 5, 4, 6), output_shape=("B", 5))
    model = build(plan, device=device)

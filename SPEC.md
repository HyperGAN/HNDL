...
- `out*` declares a variadic output expanded to `out0`, `out1`, … . It must be the sole declared output port, carries no shape pattern of its own (pair it with `relation=`), and the declaration must name the sequence argument whose length fixes the port count with `outputs_from="<argument>"` — an `"ints"` or `"strs"` argument. An empty sequence leaves the single declared port, so an operator that only sometimes fans out keeps its ordinary `out` port, its saved encoding, and its digests; any other length makes the call return a tuple of that many tensors and clear current, exactly as `split` does. Restoring a plan rechecks the stored port names against that argument (`E_BINDING`).

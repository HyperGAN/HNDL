...
**Status: the implemented 0.1.0 contract.** This document defines the technical contract for **HNDL — Human-readable Network Definition Language**, pronounced “handle.” [IMPLEMENTATION.md](IMPLEMENTATION.md) records what 0.1.0 ships and what remains for v1. [README.md](README.md) introduces the APIs with examples.
...
This operator is the documented exception to invariant 3. Its relation reads `config.json` — downloading only that file for an `hf://` source when it is not cached — and traces the architecture on PyTorch's `meta` device with the declared input shape to learn the output shape. No weights are read, no tensor is allocated, and no randomness is drawn during resolution; weights are downloaded and loaded when the network is built. The exception is confined to this operator: no other operator may read a file, reach the network, or trace a module during resolution.

...
Input, output, and node IDs match `[a-z][a-z0-9_]*`. References are `input:<name>` or `node:<id>/<port>`. Unary operators consume `x` and produce `out` by convention; every operator declares its ports explicitly, and a variadic operator expands one declared prefix into `x0`, `x1`, and so on, on either side of the arrow.
...
- Batch equality propagates through every operator whose declaration does not say otherwise; `concat(axis=0)` and `chunk(dim=0)` declare `batch="relation"` (§8) and relate their ports' batch entries themselves. Floating ports carry the plan's compute dtype unless the declaration says otherwise; no operator casts silently.
...
- Join operators require exact agreement on every axis they do not combine. For `concat`, axes index the complete tensor shape and negative axes are rejected rather than normalized. Axis `0` is batch: concatenating there stacks examples rather than widening a feature axis, so the output batch is the sum of the inputs' batches — `B + B` is `2*B` and `2*B + B` is `3*B` — and one unknown input batch is solved from a known output, exactly as one unknown extent is on any other axis. Every input must carry a whole number of plan batches; a sum beyond 1024 fails with `E_RESOURCE`.
...
### Equal sections and the batch axis

`chunk` is the equal-split counterpart of `split`: it takes input port `x`, a required `chunks`, and `dim` defaulting to `1`, and produces exactly `chunks` output ports `out0 … outN-1`, each holding `x[dim] / chunks` elements on `dim`. Every other dimension, dtype, and batch value is equal across input and outputs. The division must be exact and no section may be empty: `chunk(x, 2, dim=1)` on a width of 5 fails with `E_CONSTRAINT` at resolution, not at runtime. `1 <= chunks <= 32`; `chunks=1` is allowed and returns a one-tuple holding the whole tensor, which — like any multi-output call (§7) — clears current. Inference runs both ways: a known section extent fixes the input as `section * chunks`.

```text
a, b = chunk(x, 2, dim=1)         # [B,8] gives [B,4] and [B,4]
combined = concat(p, q, axis=0)   # [B,C,H,W] twice gives [2*B,C,H,W]
a, b = chunk(combined, 2, dim=0)  # back to [B,C,H,W] and [B,C,H,W]
```

`dim=0` divides the batch, which is the inverse of `concat(..., axis=0)`: the input's batch entry must be a whole multiple of `chunks`, and each output carries the quotient. `2*B` into two sections gives `B` each; `2*B` into three, or a plain `B` into two, fails with `E_CONSTRAINT` at resolution, because nothing establishes that the runtime batch is divisible. That check belongs to resolution rather than to a runtime that would only ever see one batch size; the module still rejects an indivisible extent with `E_RUNTIME` when it is called directly, outside the graph that checked its contract. The backend uses one exact size in `torch.split`, so the sections are views: first and second derivatives flow to the input through every one of them.

Together the two operators express one shared — often frozen — network applied to several branches in a single forward pass:

```text
combined = concat(candidate, context, axis=0)          # [2*B,3,H,W]
features = pretrained(combined, "...", output="features")
...
`from hndl import ops` exposes symbolic operators by alias. Inside a capture it resolves each alias against the registry that capture runs under, the one passed to `resolve_callable` or `network_from_callable` (the built-in catalog when none is passed), so operators registered on that registry are called as `ops.<alias>` like the built-ins. Outside a capture its aliases are checked against the built-in catalog, and calling one fails with `E_CAPTURE`. A factory read outside a capture resolves its alias again when called. `registry.ops.<alias>` is bound to that registry: it names the exact declaration registered there when the attribute is read. These calls capture graph nodes, not numerical torch operations. Native Python variables follow ordinary Python rules and never name modules. The callable is invoked exactly once per API call, with one symbolic tensor for every declared external input, even if its body never refers to those parameters. With one unnamed output a returned symbolic tensor selects it; `None` or ordinary fallthrough selects current; any other return value fails without falling back to current. With named outputs the callable must return a mapping whose keys are exactly the declared names and whose values are symbols of this capture; `None`, a bare symbol, a missing name, and an undeclared name all fail with `E_OUTPUT`. Zero-argument callables are not a separate supported convention. A symbolic tensor cannot be used as a Python truth value or substituted with an eager tensor. Ordinary Python helpers and loops may construct a finite static graph; they do not become runtime graph control flow.
...
`resolve`, `resolve_file`, `resolve_callable`, `ops`, `Registry`, `ResolvedPlan`, `HNDLError`, and the declaration types `operator`, `Arg`, `Example`, and `Policy` are exported by `hndl`; construction functions are exported by `hndl.torch`. A string always means source, never a filename or function to execute. File paths are supplied only by the host through the explicit file APIs. A missing registry selects an independent built-in registry. Explicit registry objects supply extensions without a mutable global singleton. `ops` and `registry.ops` calls both capture exact operator identities, and may be mixed in one capture when they agree. A `registry.ops` factory may run only in a capture whose registry holds that very declaration: every `Registry.builtins()` shares the built-in declarations, so another built-in registry's `ops.linear` is accepted, while a factory whose identity the capture's registry lacks, or declares with a different implementation, fails with `E_CAPTURE` naming the operator and the registry to pass. Both modes use identical operator/policy rules and output contracts. Their graph has the external inputs and public outputs the contracts declare, defaulting to input `x` and output `output`; each output's contract constrains the tensor it selects. When a configuration with one unnamed output binds `out`, its final value must be one tensor symbol and takes precedence over current; otherwise current is selected. Native `None`/fallthrough selects current, while an explicit symbol selects itself and any other value fails. An unset current fails when default selection requires it; an invalid explicit selection always fails. Empty source or a callable with no operator calls using default selection chooses the external input, subject to matching input/output contracts.
...
After a successful single-output operator call, its output becomes current. This applies to explicit-input calls, nested calls, and multi-input joins as well as implicit-input calls. A multi-output call such as `split(64)` returns its declared tuple and clears current; the next one-input operation must select an input explicitly, for example `linear(z1, 128)`. An operator with a variadic output port, such as `chunk(2)` or `pretrained(..., layers=("layer1", "layer2"))`, behaves identically, including when its argument asks for a single output. Merely unpacking or assigning `z1` does not restore current. A discarded multi-output result also leaves current unset; ordinary reachability rules still reject dead nodes.
...
- The module contains simple assignment statements with one target, expression statements consisting of a registered operator call, and bounded loops `for _ in range(N):` whose bodies contain any of these statements (see *Bounded loops* below). Targets are a local name or a flat tuple/list of distinct names for unpacking. Chained, annotated, augmented, attribute, subscript, and starred assignments fail.
...
A configuration may repeat statements with exactly one loop form:

```python
h = x
for _ in range(8):
    h = add(h, feed_forward(layer_norm(h), 512))
```

- The target is `_` and the iterator is `range(N)` with exactly one argument, a positive integer literal. A different target, `range` with zero or several arguments or a keyword, a count that is a name, expression, float, `bool`, zero or negative, any other iterable, `for ... else`, `break`, `continue`, `while`, `async for`, `if` statements and conditional expressions, and list/set/dict comprehensions and generator expressions each fail with `E_SYNTAX` and a message naming the construct.
- There is no loop variable. `_` is never a value: using it as an argument, keyword value or assignment value fails with `E_SYNTAX`, while assigning to it, as in `a, _ = split(64)`, still discards a value. `range` is only the loop iterator: calling or naming it anywhere else, or rebinding it, fails with `E_SYNTAX`, and a registry alias `range` is reserved like `x` and `out`. There is no arithmetic and no index expression. Index-dependent structure, such as widths that change per stage, belongs in trusted native Python (`resolve_callable`, `network_from_callable`), where ordinary Python loops build the same static graph.
- The body may contain every top-level statement, including assignments, rebinding, tuple unpacking and nested loops. It must call at least one operator, directly or in a nested loop, so every iteration emits a node and `max_nodes` bounds the work of unrolling as well as the graph; a body of assignments alone fails with `E_SYNTAX`. Loops nest at most 8 levels.
- Loops are unrolled while the configuration is interpreted, before resolution. Each iteration runs the body against the current local bindings, so rebinding carries from one iteration to the next and current flows through as it does in straight-line code. The resolver, backends and plan format see only ordinary nodes: a loop produces the same node IDs, arguments, edges, plan, semantic digest, `to_json()` apart from each node's source location, and `state_dict()` keys as the same statements written out by hand. Generated IDs keep counting `n0`, `n1`, … across iterations.
- An explicit `name="block"` inside a loop gains a suffix: the index of every enclosing loop, outermost first, joined by `_`. One loop gives `block0` … `block7`; `name="res"` in an inner `range(3)` inside an outer `range(2)` gives `res0_0`, `res0_1`, `res0_2`, `res1_0`, …, and a name in the outer body only gets the outer index. Indices count from 0 like `range`. Per-level indices keep an ID stable when another loop's count changes and read directly as the iteration a diagnostic reports; a single running counter would renumber every later block. Names outside loops are unchanged. A suffixed ID that equals another node's ID fails with `E_NAME`, and the message names the loop name, the suffix and the earlier node.
- The parser multiplies out each loop's node count, body calls times `N` including nested loops, and the interpreter checks it against `max_nodes` before emitting the loop's first node. `range(1000000000)` therefore fails with `E_RESOURCE` at once, allocating nothing.
- Each node unrolled from a loop records `iterations` in its source metadata next to `line` and `column`: one `[index, count]` pair per enclosing loop, outermost first. Configuration, resolution and runtime errors about the node report it after the location, for example `E_CONSTRAINT (node block3; line 12, column 5, iteration 3 of 8): ...` or, nested, `line 12, column 9, iteration 1 of 2, 3 of 4`. The index is the one the name suffix uses.

### Loading limits and trust boundaries

String and file entry points share one pipeline. Apply the byte bound before parsing; files are read as bounded UTF-8 with explicit errors for invalid encoding or oversized content, without reading an unbounded file first. Apply common-indentation removal compatible with `textwrap.dedent` so indented triple-quoted examples work. Keep a source map from dedented columns to original line/column positions. Files may not request other files, imports, or callbacks. The host supplies a path; configuration cannot choose what to load.
...
Bound the source size and the nesting depth before parsing, and bound graph nodes/edges and resolution work. Parsing itself can exhaust resources, as documented for [Python AST parsing and literal evaluation](https://docs.python.org/3/library/ast.html): on CPython 3.11–3.13 a few thousand chained operators crash `ast.parse` on a small thread stack instead of raising. The loader therefore screens the token stream first, failing more than 50 levels of bracket nesting or more than 8 levels of loop indentation with `E_RESOURCE`, and more than 32 Python operators or keywords the subset does not accept (it accepts only `True`, `False`, `None`, and the loop's `for` and `in`) with `E_SYNTAX`; any `SyntaxError`, `MemoryError` or `RecursionError` the parser still raises becomes `E_SYNTAX` or `E_RESOURCE`. Parsing runs in the host process: the AST is data and is never compiled or executed. Literal sizes need no separate bound because the source bound covers them and each operator argument schema bounds its own values. All declarative source, including file content, follows the same parser/validator path. Do not infer that a string is trusted from its origin or silently fall back to an unbounded parser. The AST allowlist is the execution boundary; it is not a denial-of-service sandbox, and hosts parsing hostile source at scale should add their own process isolation. Operator declarations remain trusted code and must not be selected from arbitrary external imports by configuration. Use bounded, explicitly declared argument schemas, not user callbacks for interpreting values. Trusted native capture remains the separately requested execution mode, not an alternate parser behavior.
...
- `B` is the batch axis and must be first. It stands for the batch entry this node's ports share, which is the plan's `B` unless a batch-axis join upstream made it a multiple such as `2*B`; a pattern never spells a multiple itself. Other entries are dimension symbols (`C`, `D_in`), exact integer multiples written `2*C`, or positive integer literals. A symbol is scoped to one node invocation: the same name on two ports or axes asserts equality in both directions, and unrelated nodes never share a symbol because both spell it `C`.
...
- `out*` declares a variadic output expanded to `out0`, `out1`, … . It must be the sole declared output port, carries no shape pattern of its own (pair it with `relation=`), and the declaration must name the argument fixing the port count with `outputs_from="<argument>"` — an `"ints"` or `"strs"` argument, whose length is the count, or an `int` argument bounded to 0…32 ports, whose value is. A count of zero leaves the single declared port, so an operator that only sometimes fans out keeps its ordinary `out` port, its saved encoding, and its digests; any other count makes the call return a tuple of that many tensors and clear current, exactly as `split` does. Restoring a plan rechecks the stored port names against that argument (`E_BINDING`).
...
- `batch="shared"`, the default, states that the operator leaves axis 0 alone: the resolver gives every port of the node the one batch entry they share, whether or not the port declares a pattern. `batch="relation"` states that the operator moves tensors across the batch axis, and its `relation=` then relates the ports' batch entries itself with `s.batch(port)` and `s.axis(port, 0, value)`. Only `concat` and `chunk` declare it.
...
`Arg(type, default, *, min, max, exclusive_min, exclusive_max, help, inferable, dim, positional, choices)` declares one scalar argument. Supported types are `int`, `float`, `bool`, `str`, `"pair"` (an int or two ints, normalized to a pair), `"ints"` (a bounded tuple of ints), and `"strs"` (a bounded tuple of strings, written as a tuple literal in configuration). Integers and booleans are distinct; float arguments accept finite integer or float literals and normalize to floats; `choices` restricts a `str`.
...
| `s.batch(port)` | The port's batch entry — `"B"`, a multiple such as `"2*B"`, or `None` |
| `s.share_batch(*ports)` | Give the named ports the one batch entry they share |
...
Referencing an undeclared port fails with `E_REGISTRY`. Relations must terminate and must be monotone; the resolver bounds sweeps with `max_iterations` and fails with `E_RESOURCE` rather than looping. Shared relations for families of operators live in the public module `hndl.relations`, which the built-ins use and custom operators may import: the convolution arithmetic `conv_output`, `conv_input_range`, `conv_transpose_output` and `conv_transpose_input`; the per-axis relations `conv_axis` and `conv_transpose_axis`; `spatial("conv2d" | "conv_transpose2d")`, the complete 2D convolution relation; `elementwise_join(*ports)`; and `broadcast(s, operation)`. Each works in both directions and adds only facts that follow from what is known. `hndl.operators._relations` re-exports them for older imports.
...
`hndl.testing` is the harness the built-in catalog runs, public so a package declaring its own operators runs the same checks against its registry: `check_operator(registry, alias, devices=None, dtypes=None)` checks the declaration and, for every example on every device, the frontend and JSON round trip, the build and run, and the declared reference; `check_declaration`, `check_round_trip`, `check_build_and_run` and `check_reference` run one check each, and `example_params(registry, aliases)` / `operator_params` yield `pytest.param`s for them. Checks raise `AssertionError`. Importing `hndl` or `hndl.testing` does not import pytest.
...
Serialization uses canonical JSON with sorted object keys, compact separators, UTF-8 without ASCII escaping, arrays for tuples, finite numbers only, and SHA-256 digests. Reading rejects duplicate JSON fields, unexpected or missing plan fields, non-finite constants, and nodes missing any canonical field (`E_SCHEMA`), then verifies both digests (`E_INTEGRITY`) and revalidates the concrete equations against the supplied registry without re-running source, a callable, or a policy choice. The digests detect corruption and hand edits, and their error says so; they are checksums, not signatures. Exact encoding and digest test vectors must be published before the format is frozen.
...
relu()
linear(init={"weight": xavier_uniform(gain=1.0), "bias": 0}, trainable={"bias": False})
```

`init` is a mapping from exact relative parameter names to either a constant number or one initializer scheme call. Omission or an empty mapping retains ordinary constructor initialization, identified in the plan as `torch_default@1`. Overrides apply after the module is materialized on the selected device, under `torch.no_grad`, within the construction RNG scope, in the module's own parameter order and then node order. Constructors still execute their normal initialization and consume their normal RNG draws. A constant override draws no randomness; a scheme override draws from the same construction RNG scope, so `initialization_seed` reproduces it exactly. Neither affects forward-time execution.

Initializer schemes are a fixed, closed set of keyword-only calls, each naming one `torch.nn.init` routine with that routine's keyword names and defaults:

| Call | `torch.nn.init` | Keywords (defaults) |
| --- | --- | --- |
| `xavier_uniform` | `xavier_uniform_` | `gain=1.0` |
| `xavier_normal` | `xavier_normal_` | `gain=1.0` |
| `kaiming_uniform` | `kaiming_uniform_` | `a=0.0`, `mode="fan_in"`, `nonlinearity="leaky_relu"` |
| `kaiming_normal` | `kaiming_normal_` | `a=0.0`, `mode="fan_in"`, `nonlinearity="leaky_relu"` |
| `truncated_normal` | `trunc_normal_` | `mean=0.0`, `std=1.0`, `a=-2.0`, `b=2.0` |
| `normal` | `normal_` | `mean=0.0`, `std=1.0` |
| `uniform` | `uniform_` | `a=0.0`, `b=1.0` |
| `orthogonal` | `orthogonal_` | `gain=1.0` |

`mode` is one of `fan_in` or `fan_out`; `nonlinearity` is one of the names `torch.nn.init.calculate_gain` accepts (`linear`, `conv1d`, `conv2d`, `conv3d`, `conv_transpose1d`, `conv_transpose2d`, `conv_transpose3d`, `sigmoid`, `tanh`, `relu`, `leaky_relu`, `selu`). Every other keyword is a number normalized exactly like a constant override. The eight names are reserved: an operator alias may not shadow one, in a registry (`E_REGISTRY`) or in a parse request (`E_NAME`).

Scheme calls are legal only as direct values of an `init` mapping. A bare `init=xavier_uniform()`, a scheme inside a list, a scheme nested in another call's arguments, and a scheme in any other keyword's dictionary are all `E_SYNTAX` in declarative source; the general rule that literal containers cannot contain calls is otherwise unchanged. In trusted Python the same names are ordinary functions, `hndl.xavier_uniform(gain=1.0)`, returning the record the parser builds, so both frontends produce identical plans.
...
Each mapping has at most 256 entries. Paths have at most 256 ASCII characters, with dot-separated nonempty identifier or numeric segments, supporting names such as `projection.weight` and `layers.0.weight`. They are lookup keys into registered parameters, never Python attribute expressions to evaluate. Wildcards, indexing expressions, and buffer targets are unsupported. Constant values for `init` must have exact type `int` or `float`, be finite, and round to finite IEEE binary32 values; booleans are rejected. The plan stores the rounded float32 value, including underflow to zero, preserving the sign of zero, and the backend fills it into the parameter's compute dtype. Numeric scheme keywords follow the same rule. Trainability values have exact type `bool`.

Capture and resolution validate these bounded declarations but cannot confirm a module's parameter inventory. The backend verifies every target after construction and before applying any overrides; missing parameters (including disabled `bias` or `affine`), buffer targets, and conflicting aliases fail. Multiple names of the same parameter must not receive conflicting explicit values or flags; two names of one parameter may carry the same scheme with the same arguments. Initialization requires a materialized parameter in the plan's compute dtype and rejects storage shared with any distinct parameter or registered buffer. `xavier_uniform`, `xavier_normal`, `kaiming_uniform`, `kaiming_normal`, and `orthogonal` require a parameter of at least two dimensions, which the backend checks before initializing any node rather than surfacing a `torch` exception; a shape a scheme cannot handle is `E_INITIALIZATION` naming the parameter and its shape. When trainability settings touch shared storage, all distinct parameters using that storage must have the same effective flag; untouched constructor defaults remain valid. A request to enable gradients on an integer parameter fails. Module/state ownership checks remain in force across nodes.

The immutable node records persist canonical construction fields separately from operator arguments:

- `initialization = {"kind": "torch_default@1" | "torch_default@2", "overrides": {...}}`. Each override is a canonical float32 constant, or, under `torch_default@2`, a scheme record `{"kind": "<scheme>", ...}` carrying every keyword of that scheme with its canonical value. A node whose overrides are all constants — including the empty default — stores `torch_default@1` unchanged, so plans written before schemes existed keep their exact bytes and digests. Both versions are read forever; `torch_default@2` requires at least one scheme record, and a scheme record under `torch_default@1` is `E_INITIALIZATION`.
...
Revalidation resolves the saved arguments again, as explicit values, against the saved external contracts, and compares the result with the saved plan. An identical result loads the plan unchanged. A result that differs only by a value the saved plan already determines loads the plan completed with that value, and warns with what was filled and the new semantic digest. Two such values exist: an argument's declared operator default (a plan written before the operator gained the argument without `since=`), and an argument bound to a dimension symbol (`Arg(dim=...)`), whose value is read off a saved port shape that has itself been verified. Every other difference fails with `E_INTEGRITY` and lists each mismatch per node: a saved port shape the equations do not give, a saved argument value the equations change or the operator no longer declares, and a missing argument that only a policy or relation search would choose again. Those are refused because accepting them would change the network a checkpoint was trained with, not just its encoding. A missing value is also refused when resolution itself fails, with the resolver's error.
...
| `E_RESOURCE` | Source size, nesting or loop depth, graph (including a loop's unrolled node count), state-size, or resolution budget is exceeded |
...
| Configuration loops | `for _ in range(N):` unrolls to the same node IDs, plan, semantic digest, `to_json()` apart from source locations, and `state_dict()` keys as the statements written by hand, for chains, residual rebinding, nested loops and named nodes; `name=` gains the per-level iteration suffix and suffix collisions fail with `E_NAME`; every other loop target, `range` form or count, `for ... else`, `break`, `continue`, `while`, `if`, comprehensions, `_` as a value, `range` elsewhere and a body without an operator call fail with `E_SYNTAX` naming the construct; a huge `N` and deep nesting fail with `E_RESOURCE` before any node is emitted, including on a small thread stack; configuration, resolution and runtime errors report the iteration; an equivalent native Python loop gives the same plan |
...
| Loading boundary | Bounded UTF-8 file reads, dedent/source mapping, source-size and nesting bounds checked before parsing, pathological nesting fails cleanly on a small thread stack, no eval/exec/import dispatch, no fallback between string/file/callable modes |
| Native capture | Trusted fn called exactly once with symbolic input; explicit symbol return or None/fallthrough current selection; helpers/static loops work; context cleanup/concurrency; cross-capture/coercion failures; no callback execution during forward/inspection/restore; `ops` resolves custom aliases against the capture's registry, mixes with `registry.ops`, and a foreign registry's factory fails with `E_CAPTURE` |
| Operator declarations | Every declared operator has a summary, docstring, category, help text on every argument, and at least one example; every example resolves identically in both frontends, round-trips through JSON, builds, and runs on every available device; operators declaring `reference` match their handwritten equivalent; the generated catalog is current (`python -m hndl.docs --check`); the built-in suite runs these checks through the public `hndl.testing` harness, and an operator declared outside the package passes the same harness |
...
| Equal sections | `chunk` divides an axis into exactly `chunks` equal sections and infers the input extent from one of them; an uneven division and an out-of-range `dim` are rejected at resolution; `chunks=1` returns a one-tuple that clears current; sections are views whose first and second derivatives reach the input; the port list and the argument round-trip together |
| Batch axis | `concat(axis=0)` reports `B + B` as `2*B` and `2*B + B` as `3*B`, solving one unknown input batch from a known total; `chunk(dim=0)` inverts it; intermediate contracts track `k*B` separately from `B` through ordinary operators and through `pretrained`, including `layers=`; one built module runs at batches 1, 3 and 64 with correct first and second derivatives to every input; an uneven batch division, a `k*B` external contract, and a malformed saved entry such as `"0*B"`, `"B*2"` or `"2*C"` each fail with their code; a plan that never touches the batch axis keeps its encoding and digests |
...
| Variadic outputs | An operator declaring `out*` with `outputs_from=` expands ordinal output ports from that argument's length; both frontends unpack the tuple, a mismatched arity fails, the call clears current (including a one-entry sequence), an empty sequence keeps the single declared port and its digest, the plan round-trips, and a stored port list inconsistent with the argument fails with `E_BINDING` |
| Pretrained layers | `layers=` returns one output per named submodule from a single forward pass, each at its native shape and equal to a manual hook; every output carries input gradients and second derivatives through in-place activations; the pass stops after the last requested submodule; a submodule that runs twice, a duplicate, an empty, or an unknown entry, and a conflict with `layer=` or `readout=` each fail with `E_PRETRAINED`; the plan round-trips while a plan without `layers=` keeps its single `out` port |
...
- Qualify the supported platform, PyTorch version, and device/dtype matrix with numerical tolerances.
